...
def merge_aggregation(dataframe, keyfield, shapefile, zonekey):
    """
    ...
    try:
        ...
        log.exception('Cannot import geopandas module - exiting')
        raise

    dataframe[keyfield] = dataframe[keyfield].astype(str)
    gdf = geopandas.read_file(shapefile)
    gdf[zonekey] = gdf[zonekey].astype(str)
    mergedf = pd.merge(gdf, dataframe, how='inner',
                       right_on=keyfield,
                       left_on=zonefield)
